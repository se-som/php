 ...
 */
function GetAuthToken($admin_email,
                      $admin_password,
                      $captcha_token,
                      $captcha_response) {

  $post_url = "https://www.google.com/accounts/ClientLogin";
  $post_data = "accountType=HOSTED&Email=" . $admin_email .
  ...
  } else if ( $result && preg_match('/Error=CaptchaRequired/', $result)
      && preg_match('/CaptchaToken=(\S+)/', $result, $token)
      && preg_match('/CaptchaUrl=(\S+)/', $result, $url) ) {

    $full_url = "https://www.google.com/accounts/" . $url[1];
    return "Authentication failed. Please retry with token " .
      $token[1] . " and the text you see at " . $full_url;
  } else {
    return $result;
  }
}
